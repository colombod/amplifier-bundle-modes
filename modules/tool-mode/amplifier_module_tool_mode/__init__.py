...
class ModeTool:
    ...
    catches unexpected failures and returns a clean ToolResult rather than letting
    exceptions escape to the LLM. hooks-mode uses the same outer try/except shape
    for its hook handlers, with fail-closed (deny) for security-decisioned handlers
    and fail-open (continue) for context injection.
    """
